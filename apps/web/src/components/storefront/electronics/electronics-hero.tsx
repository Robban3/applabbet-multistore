...
/** Komplett/Webhallen-stil hero: full-bleed bannerbild med textoverlay. */
export function ElectronicsHero({
  ...
  return (
    <section className="bg-white px-4 py-4 sm:px-6">
      <div className="relative mx-auto min-h-[360px] max-w-[1280px] overflow-hidden rounded-[20px] bg-gradient-to-br from-[#EAF1FB] to-[#DCE6F5] lg:min-h-[460px]">
        {heroImageUrl ? (
        ...
        <div className="relative flex min-h-[360px] flex-col justify-center px-8 py-12 lg:min-h-[460px] lg:px-14">
          <div className="max-w-[520px]">
            ...
            {description ? <p className="mt-4 max-w-[460px] text-[17px] text-[#0A2540]/75">{description}</p> : null}
            <div className="mt-7 flex flex-wrap gap-3">
              ...
              {secondaryCtaHref && secondaryCtaLabel ? (
                <Link href={secondaryCtaHref} className="inline-flex h-12 items-center rounded-full border border-[#0A2540]/25 bg-white/70 px-7 text-[15px] font-semibold text-[#0A2540] transition hover:bg-white">
                  {secondaryCtaLabel}
                </Link>
              ) : null}
            </div>
