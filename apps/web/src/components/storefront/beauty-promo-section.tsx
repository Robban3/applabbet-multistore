type BeautyPromoCard = { title: string; text: string; cta: string };

export function BeautyPromoSection({ cards }: { cards: BeautyPromoCard[] }) {
  return (
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {cards.map((card, index) => (
          ...
          >
            ...
            <button type="button" className="mt-4 inline-flex items-center rounded-full border border-slate-300 bg-white px-3 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50">
              {card.cta}
            </button>
          </article>
        ))}
