...
function TrustIcon({ index }: { index: number }) {
  if (index === 0) {
    return (
      <svg viewBox="0 0 24 24" className="h-4 w-4 text-[#c8a164]" fill="none" stroke="currentColor" strokeWidth="1.9">
        <path d="M2 6h11v9H2z" />
      ...
      <div className="overflow-hidden rounded-[18px] border border-[#e3d8cc] bg-white shadow-[0_6px_24px_rgba(21,17,12,0.06)]">
        ...
        <div className="px-6 py-5">
          ...
          <div className="grid gap-5 lg:grid-cols-[1.45fr_0.85fr]">
            <section className="rounded-xl border border-[#e6ddd1] bg-white p-3">
              {items.length === 0 ? (
                ...
                <div className="divide-y divide-[#ece4d8]">
                  {items.map((item) => {
                    ...
                    return (
                      <article key={item.productId} className="grid grid-cols-[150px_1fr_auto_auto_auto] items-center gap-4 py-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-600">{details.variant}</p>
                        ...
                        <div className="inline-flex items-center rounded-md border border-slate-300">
                          <button onClick={() => updateQuantity(item.productId, -1)} className="px-3 py-1.5 text-slate-700">−</button>
                        ...
                        <button onClick={() => removeItem(item.productId)} className="text-lg text-slate-400 hover:text-slate-600">×</button>
                      </article>
                    );
                  })}
                </div>
            ...
            <aside className="space-y-3 rounded-xl border border-[#e6ddd1] bg-white p-5">
              ...
              <Link href="/checkout" className="block w-full rounded-md bg-[#c8a164] px-4 py-3 text-center text-sm font-semibold text-slate-900 transition hover:bg-[#b88f50]">
                TILL KASSAN
              </Link>
